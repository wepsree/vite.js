		...
		it(name, function () {
			switch (name) {
				case 'receive': {
					...
					assert.deepEqual(
						{ address, blockType, sendBlockHash },
						{
							address: _address,
							blockType: 4,
							sendBlockHash: '18095215a419e346dde2f678180382b65146502fb543a7e96c41b141136b47d9',
				...
				case 'send': {
					...
					const { address, blockType, toAddress, tokenId, amount, data } = createAccountBlock(
						name,
						params
					);
					assert.deepEqual(
						{ address, blockType, toAddress, tokenId, amount, data },
						{
							address: _address,
							blockType: 2,
							toAddress: _address,
				...
				case 'constructor': {
					...
					assert.throws(
						() => createAccountBlock(name, params),
						new Error(`Don\'t support transaction type ${name}`)
					);
					break;
				}
			}
