...
async function SendTxToMyself(previousAccountBlock) {
	...
	const data = await accountBlock
		.setPreviousAccountBlock(previousAccountBlock)
		.sendByPoW(privateKey);
	console.log('[LOG] SendTxToMyself', data, '\n');
	return data;
}
...
async function CheckTokenList(previousAccountBlock) {
	...
	if (toChangeTransferShipOne) {
		await sleep(2000);
		previousAccountBlock = await TransferTokenOwnership(
			previousAccountBlock,
			toChangeTransferShipOne
		);
	}
