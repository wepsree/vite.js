	...
	_parse(data) {
		...
		results.forEach((ele) => {
			if (!(ele instanceof Array) && !ele.id) {
				// not an array and has no json-rpc id, call the registered subscription event handler
				this.subscribeMethod && this.subscribeMethod(ele);
				return;
			}

			if (ele.id) {
				// has json-rpc id, meaning this is not subscription callback, call the corresponding response callback by id
