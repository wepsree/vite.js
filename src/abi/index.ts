...
export function encodeLogSignature(
	abiFragment: Array<AbiFragment> | AbiFragment,
	eventName?: string
): string {
...
export function encodeOffchainCall(
	abiFragment: Array<AbiFragment> | AbiFragment,
	params?: Array<any>,
	methodName?: string
) {
...
export function decodeOffchainOutput(
	abiFragment: Array<AbiFragment> | AbiFragment,
	data: string,
	methodName?: string
) {
...
export function encodeConstructor(
	abiFragment: Array<AbiFragment> | AbiFragment,
	params?: Array<any>
) {
...
export function encodeParameters(
	types: Array<AbiParam | AbiFragment> | AbiParam | AbiFragment,
	params?: Array<any>,
	methodName?: string
) {
...
export function encodeLogFilter(
	abiFragment: Array<AbiFragment> | AbiFragment,
	values: Array<any>,
	eventName?: string
) {
...
function getTypes(
	types: Array<ParamType | AbiFragment> | ParamType | AbiFragment,
	methodName?: string
): Array<ParamType> {
	...
	if (methodName) {
		// fragment array + methodName
		const result = _frags.find(
			(item) => item.name === (item.type === 'callback' ? `${methodName}Callback` : methodName)
		)?.inputs;
