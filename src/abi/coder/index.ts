	...
	unpack(reader: Reader, coders: Array<Coder>): Result {
		...
		coders.forEach((coder: Coder, index: number) => {
			...
			if (this.name === 'tuple' && this.localName !== '_') {
				...
				// TODO: enable to add name index to decode result
		...
		if (
			this.name === 'tuple' &&
			this.localName !== '_' &&
			Object.keys(tuple).length === values.length
		) {
			values = tuple;
		}
	...
	_peekBytes(offset: number, length: number, loose?: boolean): Buffer {
		...
		if (this._offset + alignedLength > this._data.length) {
			if (this.allowLoose && loose && this._offset + length <= this._data.length) {
				...
				throw new Error(
					`data out-of-bounds: ${this._offset + alignedLength}, actual: ${this._data.length}`
				);
			}
		}
