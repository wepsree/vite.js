...
function isValidVersion3(keyJson) {
	// Required parameter
	if (!keyJson.uuid || !keyJson.crypto || !keyJson.version || Number(keyJson.version) !== 3) {
		return false;
	}
...
export default function isValid(keystore) {
	...
	try {
		...
		if (
			(keyJson.version && Number(keyJson.version) > currentVersion) ||
