		...
		[
			{
				...
				func: (_originalAddress) =>
					typeof _originalAddress === 'string' &&
					/^[0-9a-fA-F]+$/.test(_originalAddress) &&
					(_originalAddress.length === ADDR_SIZE * 2 ||
						_originalAddress.length === (ADDR_SIZE + 1) * 2),
			},
		]
	);
	if (err) {
