...
export const Default_Contract_TransactionType: { readonly [key: string]: ContractInfo } =
	encodeContractList(Contracts);
 ...
 */
export function checkAccountBlock(
	accountBlock: {
		blockType: BlockType;
		address: Address;
		fee?: BigInt;
	...
	if (
		(accountBlock.difficulty && !accountBlock.nonce) ||
		(!accountBlock.difficulty && accountBlock.nonce)
	) {
		return {
			code: paramsMissing.code,
	...
	const address = getAddressFromPublicKey(
		Buffer.from(accountBlock.publicKey, 'base64').toString('hex')
	);
...
export function createContractAddress({
	...
	const _o: Buffer = Buffer.concat(
		[originAddressBuffer, heightBuffer, previousHashBuffer],
		totalLength
	);
...
export function getCreateContractData({
	...
	const err = checkParams(
		{ responseLatency, quotaMultiplier, randomDegree, code },
	...
	let data = `${Delegate_Gid}01${Buffer.from(_responseLatency).toString('hex')}${Buffer.from(
		_randomDegree
	).toString('hex')}${Buffer.from(_quotaMultiplier).toString('hex')}${code}`;
 ...
 * @param methodName
 */
export function getCallContractData({
	abi,
	params,
	methodName,
}: {
...
// Sign
export function signAccountBlock(
	accountBlock: {
		blockType: BlockType;
		address: Address;
		hash?: Hex;
	...
	const { address, publicKey } = createAddressByPrivateKey(privateKey);
	if (accountBlock.address !== address) {
 ...
 */
export function decodeContractAccountBlock({
	accountBlock,
	contractAddress,
	abi,
	topics = [],
	...
	const err = checkParams(
		{ accountBlock, contractAddress, abi },
		['accountBlock', 'contractAddress', 'abi'],
		[
			{
	...
	if (
		accountBlock.blockType !== BlockType.TransferRequest &&
		accountBlock.blockType !== BlockType.Response
	) {
		throw new Error(`wrong block type: ${accountBlock.blockType}`);
	}
	if (accountBlock.toAddress !== contractAddress) {
		throw new Error(
			`contract address mismatch, expected: ${contractAddress}, actual: ${accountBlock.toAddress}`
		);
	}
 ...
 * @param methodName Function or event name
 */
export function decodeAccountBlockDataByContract({
	data,
	abi,
	topics = [],
	methodName,
 ...
 */
export function encodeContractList(contractList: { [name: string]: ContractInfo }): {
...
export function getTransactionType(
	...
	const defaultType = {
		...
		abi: undefined,
		transactionType: BlockType[blockType],
	};
	...
	const allContractTransactionType = Object.assign(
		{},
		contractTransactionType || {},
