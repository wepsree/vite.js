...
class AccountBlockClass {
	...
	async getPreviousAccountBlock(): Promise<AccountBlockType> {
		const previousAccountBlock: AccountBlockType = await this.provider.request(
	...
	setPreviousAccountBlock(previousAccountBlock: AccountBlockType): AccountBlockClass {
		let height: Uint64 =
			previousAccountBlock && previousAccountBlock.height ? previousAccountBlock.height : '';
		height = height ? new BigNumber(height).add(new BigNumber(1)).toString() : '1';
		this.setHeight(height);

		const previousHash: Hex =
			previousAccountBlock && previousAccountBlock.hash ? previousAccountBlock.hash : Default_Hash;
