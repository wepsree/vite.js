...
class AccountClass {
	...
	createContract({
		...
		const err = checkParams(
			{ abi, responseLatency, quotaMultiplier, randomDegree },
	...
	registerSBP({
		...
		const err = checkParams(
			{ blockProducingAddress, sbpName, rewardWithdrawAddress },
			['blockProducingAddress', 'sbpName', 'rewardWithdrawAddress'],
			[
				{
					name: 'sbpName',
	...
	revokeSBP({ sbpName }: { sbpName: string }): AccountBlock {
		const err = checkParams(
			{ sbpName },
			['sbpName'],
			[
	...
	voteForSBP({ sbpName }: { sbpName: string }): AccountBlock {
		const err = checkParams(
			{ sbpName },
			['sbpName'],
			[
	...
	cancelQuotaStake({ id }: { id: Bytes32 }): AccountBlock {
	...
	issueToken({
		...
		const err = checkParams({ tokenName, tokenSymbol, decimals }, [
			'tokenName',
			'tokenSymbol',
			'decimals',
		]);
	...
	burnToken({ tokenId, amount }: { tokenId: TokenId; amount: BigInt }): AccountBlock {
	...
	disableReIssueToken({ tokenId }: { tokenId: TokenId }): AccountBlock {
	...
	dexDeposit({ tokenId, amount }: { tokenId: TokenId; amount: BigInt }): AccountBlock {
		const err = checkParams({ tokenId, amount }, ['tokenId', 'amount']);
		if (err) {
			throw err;
		}
	...
	dexWithdraw({ tokenId, amount }: { tokenId: TokenId; amount: Uint256 }): AccountBlock {
		const err = checkParams({ tokenId, amount }, ['tokenId', 'amount']);
		if (err) {
			throw err;
		}
	...
	dexCancelOrder({ orderId }: { orderId: Hex | Base64 }): AccountBlock {
		const err = checkParams(
			{ orderId },
			['orderId'],
			[
	...
	dexStakeForMining({ actionType, amount }: { actionType: Uint8; amount: Uint256 }): AccountBlock {
		const err = checkParams(
			{ actionType, amount },
			['actionType', 'amount'],
			[
	...
	dexStakeForVIP({ actionType }: { actionType: Uint8 }): AccountBlock {
		const err = checkParams(
			{ actionType },
			['actionType'],
			[
	...
	dexBindInviteCode({ code }: { code: Uint32 }): AccountBlock {
	...
	dexStakeForSuperVIP({ actionType }: { actionType: Uint8 }): AccountBlock {
		const err = checkParams(
			{ actionType },
			['actionType'],
			[
	...
	dexLockVxForDividend({ actionType, amount }: { actionType: Uint8; amount: Uint256 }) {
		const err = checkParams(
			{ actionType, amount },
			['actionType', 'amount'],
			[
	...
	dexSwitchConfig({ switchType, enable }: { switchType: Uint8; enable: boolean }) {
	...
	dexStakeForPrincipalSVIP({ principal }: { principal: Address }) {
		const err = checkParams(
			{ principal },
			['principal'],
			[
