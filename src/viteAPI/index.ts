...
class ViteAPIClass extends Provider {
	...
	addTransactionType(contractList: { [name: string]: ContractInfo } = {}) {
		for (const transactionType in contractList) {
			if (Contracts[transactionType]) {
				throw new Error(
					`Please rename it. Your transactionType ${transactionType} conflicts with default transactionType.`
				);
			}
		}

		const transactionTypeAfterEncode = encodeContractList(contractList);
		this.customTransactionType = Object.assign(
			{},
			this.customTransactionType,
			transactionTypeAfterEncode
		);
	}
	...
	async getTransactionList(
		...
		const err = checkParams(
			{ address, pageIndex, decodeTxTypeList },
			['address', 'pageIndex'],
			[
				{
					name: 'address',
		...
		rawList.forEach((accountBlock: AccountBlockType) => {
			...
			const { abi, transactionType, contractAddress } = getTransactionType(
				accountBlock,
				this.customTransactionType
			);
	...
	async getNonce({
		...
		const err = checkParams({ difficulty, previousHash, address }, [
			'address',
			'difficulty',
			'previousHash',
		]);
